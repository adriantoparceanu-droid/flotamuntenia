<div>
    <x-slot name="header">
        <h2 class="flex items-center gap-2 font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <x-heroicon-o-receipt-percent class="w-6 h-6 text-indigo-600" />
            Cote TVA
        </h2>
    </x-slot>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                <th class="px-4 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Cota (%)</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                            @forelse($cote as $tva)
                                <tr>
                                    <td class="px-4 py-2 text-gray-900 dark:text-gray-100">{{ number_format($tva->valoare, 2) }}</td>
                                    <td class="px-4 py-2 text-gray-900 dark:text-gray-100">{{ $tva->denumire }}</td>
                                    <td class="px-4 py-2 text-center">
                                        <button wire:click="comutaActiv({{ $tva->id }})"
                                                class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium {{ $tva->activ ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                                            @if($tva->activ)
                                    ...
                                    <td class="px-4 py-2 text-right">
                                        <button wire:click="editeaza({{ $tva->id }})"
        ...
        <div x-show="deschis"
            ...
            <form wire:submit.prevent="salveaza" class="p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
                    {{ $editandId ? 'Editare cota TVA' : 'Adauga cota TVA' }}
                </h3>

                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cota (%)</label>
                        <input type="number" step="0.01" min="0" max="100" wire:model="valoare"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100 text-sm" />
                        @error('valoare') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Denumire</label>
                        <input type="text" wire:model="denumire" maxlength="50"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100 text-sm" />
